...
dizzy_reaction Node
----------------
This node tells the robot to act dizzy.
"""
...
class DizzyReaction(Node):
    """
    A node that tells the robot to act dizzy.
    """

    def __init__(self):
        super().__init__('dizzy_reaction')
        self.e_stop = Event()
    ...
    def run_loop(self):
        ...
        while angular_vel < 0.3:
            ...
            sleep(30 * delta_ang * abs(angular_vel))
            angular_vel += delta_ang
        self.drive(linear=0.0, angular=0.6)
        sleep(0.51 * math.pi / abs(angular_vel))
        self.drive(linear=0.0, angular=0.0)
...
def main():
    rclpy.init()
    dizzy_reaction = DizzyReaction()
    rclpy.spin(dizzy_reaction)
